...
@section('content')
	...
	<form class="form-horizontal" method="POST" action="{{url('admin/products')}}" enctype="multipart/form-data">
		...
		<div class="form-group{{$errors->has('handling_time') ? ' has-error' : ''}}">
			...
			<div class="col-md-4">
				<input type="number" min="1" name="handling_time" class="form-control" value="{{old('handling_time')}}">
		...
		<div class="form-group{{$errors->has('expiring_date') ? ' has-error' : ''}}">
			...
			<div class="col-md-4">
				<input type="date" name="expiring_date" class="form-control" value="{{old('expiring_date')}}">
